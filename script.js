...
function translateDynamicContent() {
    ...
    document.querySelectorAll('.equipment-info h3').forEach(heading => {
        ...
        const key = `equipment_${headingText.replace(/\s+/g, '_').toLowerCase()}`;
        if (tr[key]) {
    ...
    document.querySelectorAll('.equipment-specs').forEach(spec => {
        ...
        const specKey = `spec_${specText.split(':')[0].trim().replace(/\s+/g, '_').toLowerCase()}`;

        if (tr[specKey]) {
            // Keep the measurement values, just translate the label
            const valueText = specText.includes(':') ? ': ' + specText.split(':')[1].trim() : '';
    ...
    document.querySelectorAll('.portfolio-item h3').forEach(title => {
        ...
        const titleKey = `portfolio_${titleText.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()}`;
